...
# Creamos la clase del arbol (en este caso binario) para agregarle los datos
class ArbolBinario:     
    def __init__(self):     
        self.padre = None     
    ...
    def Añadirnodo(self, valor):     
        if self.padre is None:     
            self.padre = Nodo(valor, ANCHO//2, 50)     
        else:     
            self.AgregarNodo(valor, self.padre, ANCHO//2, 60, 1)     
    def AgregarNodo(self, valor, nodo, x, y, level):     
        if nodo.izquierda is None:     
            nodo.izquierda = Nodo(valor, x-2**(6-level), y+NODO_TAMAÑO*2)     
        elif nodo.derecha is None:     
            nodo.derecha = Nodo(valor, x+2**(6-level), y+NODO_TAMAÑO*2)     
        else:     
            if valor < nodo.valor:     
                self.AgregarNodo(valor, nodo.izquierda, x-2**(6-level), y+NODO_TAMAÑO*3, level+1)     
            else:     
                self.AgregarNodo(valor, nodo.derecha, x+2**(6-level), y+NODO_TAMAÑO*3, level+1)     
